...
class SAC(modelFreeAgent.ModelFreeAgent):
    ...
    def __init__(self, *args):
        ...
        self.actor_network = self.actorModel(empty_state)
    ...
    def actorModel(self, state):
        ...
        return squashed_actions[0], logprob
    ...
    def remember(self, state, action, reward, new_state, done=False):
        ...
        action, action_logprob2 = self.actorModel(states)
        print(states.shape)
        print(actions.shape)
        print(self.allBatchMask.shape)

        value_target1 = self.softq_target_network(states, actions)
        value_target2 = self.softq_target_network2(states, actions)
        ...
        with tf.GradientTape() as softq_tape:
            softq = self.softq_network(states, self.allBatchMask)
        ...
        with tf.GradientTape() as softq_tape2:
            softq2 = self.softq_network2(states, self.allBatchMask)
            softq_loss2 = tf.reduce_mean(tf.square(softq2 - softq_targets))

        # Gradient ascent for the policy (actor)
        with tf.GradientTape() as actor_tape:
            actions, action_logprob = self.actorModel(states)
            new_softq = tf.math.minimum(self.softq_network(states, actions), self.softq_network2(states, actions))
        ...
        actor_gradients = actor_tape.gradient(actor_loss, self.actorModel.trainable_weights)
        ...
        self.actor_optimizer.apply_gradients(zip(actor_gradients, self.actorModel.trainable_weights))
    ...
    def predict(self, state: np.ndarray, isTarget):
        ...
        if isTarget:
            ...
            result1, result2 = self.actorModel(state)

        return result1, result2

    def save(self, filename):
        mem = self.actorModel.get_weights()
    ...
    def memsave(self):
        return self.actorModel.get_weights()
